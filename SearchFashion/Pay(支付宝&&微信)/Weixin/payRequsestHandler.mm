...
-(NSString *)sendPrepay:(NSMutableDictionary *)prePayParams
{
    ...
    NSString *send      = [self genPackage:prePayParams];
        NSLog(@"lllllllllllllll%@",prePayParams);
    //输出Debug Info
...
//            NSLog(@"ghghghghghgg%@==%@",resParams,send_sign);
...
- ( NSMutableDictionary *)sendPay_demoWithOrderName:(NSString *)order_n
                                           ...
                                           prepayid:(NSString *)prepayid
{
    ...
    //订单标题，展示给用户
    NSString *order_name    = order_n;
    //订单金额,单位（分）
   float p = [price floatValue] * 100;
    
//    NSLog(@"ccccccc%f",p);

    NSString *order_price   = [NSString stringWithFormat:@"%.2f",p];//1分钱测试
